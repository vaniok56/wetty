...
function friendlyMessage(reason) {
    if (!reason)
        return 'Connection closed.';
    if (reason.includes('Timed out due to inactivity')) {
        return 'Disconnected after 30 minutes of inactivity.';
    }
    if (/could not resolve hostname|connection refused|connection timed out|no route to host|operation timed out/i.test(reason)) {
...
export function disconnect(reason) {
    ...
    if (!_.isNull(msg))
        msg.innerHTML = friendlyMessage(reason || '');
    if (!_.isNull(details)) {
